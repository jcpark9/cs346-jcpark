   ...
   while (rc1 != RM_EOF || rc2 != RM_EOF) {
      ...
      if (!(rc2 == RM_EOF)) 
         if ((rc2 = rec2.GetData((char *&) data2)))
            return (-1);
      //cout << data1->str << " " << data1->num << " " << data1->r << endl;
...
{                  
   ...
   for (rc = GetNextRecScan(fs, rec); 
      ...
      //cout << counter << ":" << pageNum << "-" << slotNum << endl; 
   ...
   printf("\n*** Reuse Space (Add to new pages) Test: %s\n", 
         (rc = AddRecs(fh, LESS_RECS)) ? "FAIL\a" : "PASS");
   cout << rc << endl;
   ...
   printf("\n*** File Close Test: %s\n", 
         (rc = CloseFile(FILENAME, fh)) ? "FAIL\a" : "PASS");     
   cout << rc << endl;
